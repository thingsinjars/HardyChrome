...
(function createChannel() {
    ...
    port.onMessage.addListener(function (message) {
        if(message.action === 'scenario') {
            ghost.add.scenario(message.content);
            ghost.update.feature();
        }
        if(message.action === 'init') {
            ghost.init();
        }
        if(message.action === 'selector') {
            ghost.add.selector(message.content);
            ghost.update.selector();
        }

        if(message.action === 'setup') {
            ghost.update.scenario(message.content);
        }
